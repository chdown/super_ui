...
class _SuperExpandableTextState extends State<SuperExpandableText> {
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(
      builder: (context, size) {
        ...
        final tp = TextPainter(text: span, maxLines: widget.maxLines, textDirection: TextDirection.ltr, ellipsis: 'EllipseText');
  ...
  InlineSpan _foldButtonSpan(context) {
    return TextSpan(
        text: ' ${widget.collapseText ?? "收起"}',
        style: TextStyle(
          color: widget.collapseColor ?? Theme.of(context).primaryColor,
          fontSize: 14,
        ),
